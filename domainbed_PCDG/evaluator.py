...
def accuracy_from_loader(algorithm, loader, weights, debug=False, is_TTA=False):
    correct = 0
    total = 0
    losssum = 0.0
    weights_offset = 0
    ...
    for i, batch in enumerate(loader):
        ...
        if is_TTA:
            with torch.no_grad():
                print('x = ', x.size())
                logits = algorithm.tta_predict(x)
...
class Evaluator:
    def __init__(
        self, test_envs, eval_meta, n_envs, logger, evalmode="fast", debug=False, is_TTA=False, target_env=None
    ):
    ...
    def evaluate(self, algorithm, ret_losses=False):
        ...
        for name, loader_kwargs, weights in self.eval_meta:
            ...
            acc, loss = accuracy(algorithm, loader_kwargs, weights, debug=self.debug, is_TTA=self.is_TTA)
            accuracies[name] = acc
            losses[name] = loss
